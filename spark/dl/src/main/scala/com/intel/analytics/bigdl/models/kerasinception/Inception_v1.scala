...
package com.intel.analytics.bigdl.models.kerasinception
...
//  def apply(inputSize: Int, config: Table, namePrefix : String = "") : Sequential[Float] = {
//    val concat = Concat(2)
...
object Inception_v1_NoAuxClassifier {
  def apply(classNum: Int, hasDropout: Boolean = true): Sequential[Float] = {
    ...
    model.add(InputLayer(inputShape = Shape(3, 224, 224)))
    model.add(Convolution2D(64, 7, 7, subsample = (2, 2), padH = 3, padW = 3, propagateBack = false)
      .setName("conv1/7x7_s2"))
    model.add(Activation("relu").setName("conv1/relu_7x7"))
    model.add(MaxPooling2D(poolSize = (3, 3), strides = (2, 2),
      ceil = true).setName("pool1/3x3_s2"))
