...
object Train {
  ...
  def main(args: Array[String]): Unit = {
    trainParser.parse(args, new TrainParams()).map(param => {
      ...
      model.compile(optimizer = optimMethod, loss = TimeDistributedCriterion[Float](
        CrossEntropyCriterion[Float](), sizeAverage = true),
        metrics = Array(new Loss[Float](
          TimeDistributedCriterion[Float](CrossEntropyCriterion[Float](),
            sizeAverage = true)).asInstanceOf[ValidationMethod[Float]]))
      model.fit(trainSet, epochs = param.nEpochs, validationSet)
