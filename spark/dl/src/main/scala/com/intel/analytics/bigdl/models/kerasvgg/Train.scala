...
//  LoggerFilter.redirectSparkInfoLogs()
  ...
  def main(args: Array[String]): Unit = {
    trainParser.parse(args, new TrainParams()).map(param => {
      ...
      val model = VggForCifar10(classNum = 10)
...
//        criterion = new ClassNLLCriterion[Float](logProbAsInput = false)
//      )
      ...
      model.compile(optimizer = optimMethod,
        ...
        metrics = Array(new Top1Accuracy[Float].asInstanceOf[ValidationMethod[Float]]))
      model.fit(trainDataSet, param.maxEpoch, validateSet)
