...
if __name__ == "__main__":
    ...
    model = AutoModelForCausalLM.from_pretrained(
        r"D:\llm-models\Llama-2-7b-chat-hf",
    ...
    decoders = []
    past_key_value = DynamicFusedNormalCache()
    cache_kwargs = {"max_seq_len": 1023, "transpose": False}
    ...
    for layer_idx in range(0, 32):
        ...
        if layer_idx == 0:
            ...
            xml_path = "Llama2-7B-rest.xml"
            single_decoder.save(xml_path)
            new_ir_path = "Llama2-7B-rest-new.xml"
            blob_path = "Llama2-7B-rest.blob"
    ...
    xml_path = "Llama2-7B-lm_head.xml"
    new_lm_head.save(xml_path)
    new_ir_path = "Llama-7B-lm_head_new.xml"
    blob_path = "Llama2-7B-lm_head.blob"
    # update names for IR and export to a blob
    update_names_of_IR_and_export_blob(xml_path, new_ir_path, blob_path)
    ...
    xml_path = "Llama2-7B-embedding.xml"
    new_embedding.save(xml_path)
    new_ir_path = "Llama2-7B-embedding_new.xml"
    blob_path = "Llama2-7B-embedding.blob"
    # update names for IR and export to a blob
    update_names_of_IR_and_export_blob(xml_path, new_ir_path, blob_path)
